...
def compute_alpha(U, alpha, csv_polar):
    ...
    subprocess.run(["mkdir", "-p", "AOA_" + str(alpha)])
    subprocess.run(["cp", "-r", "Cas_de_base/0", "Cas_de_base/constant", "Cas_de_base/system", "AOA_" + str(alpha)])
    subprocess.run(["cp", "-r", "mesh.msh", "AOA_" + str(alpha)])

    # Convert the mesh from gmsh to blockMesh format
    subprocess.run(['gmshToFoam mesh.msh'], shell=True, cwd="AOA_" + str(alpha))

    # Modify the boundary dictionary
    f = open("AOA_" + str(alpha) + '/constant/polyMesh/boundary', 'r+')
    t = f.readlines()
    new_lines = []
    replace = [
        '        type            empty;\n', 
    ...
    f.close()
    f = open("AOA_" + str(alpha) + '/constant/polyMesh/boundary', 'w')
    ...
    for line in t:
        if 'uniform' in line:
            ...
            new_lines = np.append(new_lines, line)
    f.close()
    f = open("AOA_" + str(alpha) + '/0/U', 'w')
    f.writelines(new_lines)
    f.close()

    # Change the direction of liftDir/dragDir
    f = open("AOA_" + str(alpha) + '/system/controlDict', 'r+')
    t = f.readlines()
    new_lines = []
    for line in t:
        if 'liftDir' in line:
            ...
            new_lines = np.append(new_lines, line)
    f.close()
    f = open("AOA_" + str(alpha) + '/system/controlDict', 'w')
    f.writelines(new_lines)
    f.close()

    # Run simpleFoam
    subprocess.run(['foamRun > log'], shell=True, cwd="AOA_" + str(alpha))

    # Read the last lines of the file forceCoeffs.dat
    force_coeffs_path = "AOA_" + str(alpha) + "/postProcessing/forces/0/forceCoeffs.dat"
